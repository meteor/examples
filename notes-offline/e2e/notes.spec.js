...
test.describe("Notes Offline", () => {
  ...
  test("should create a new note with the button", async ({ page }) => {
    ...
    await expect(
      page.getByPlaceholder("Start writing... (supports Markdown)")
    ).toBeVisible();
  });
  ...
  test("should edit note title and body", async ({ page }) => {
    ...
    const body = `Body ${uid()}`;
    await page
      .getByPlaceholder("Start writing... (supports Markdown)")
      .fill(body);
  ...
  test("should pin a note and it floats to top", async ({ page }) => {
    ...
    await page.locator("button").filter({ has: page.locator('[class*="tabler-icon-pin"]') }).first().click();
  ...
  test("should recover a note from trash", async ({ page }) => {
    ...
    await expect(sidebar.locator(".mantine-Card-root")).toHaveCount(
      trashCountBefore - 1
    );
  ...
  test("should toggle dark and light mode", async ({ page }) => {
    ...
    const themeToggle = page.locator("button").filter({
      has: page.locator(
        '[class*="tabler-icon-sun"], [class*="tabler-icon-moon"]'
      ),
    });
